...
% 画出周期性结构 2
for i = -1 : -1 : -num_of_parallelograms
    % 平移平行四边形顶点
    shifted_parallelogram = bsxfun(@plus, parallelogram, [i * dx; 0]);
...
text(a2(1) - 0.55, a2(2) - 0.15, '$\mathbf{a_2}$', 'Interpreter', 'latex', 'FontSize', 14, 'Color', 'm');

% 用省略号表示无限延伸
text(num_of_parallelograms * dx + 1 * dx, 0.6 * dy, '...', 'FontSize', 20);
text(-num_of_parallelograms * dx - 0.3 * dx, 0.6 * dy, '...', 'FontSize', 20);

% 标注 0 的位置
% text(-0.1, -0.1, '0', 'FontSize', 12);
  
% 标注左边界
% text(0.1 * dx, 0.6 * dy, '\partial\Omega_1', 'FontSize', 12);
